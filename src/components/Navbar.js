...
function Navbar(props) {
            ...
            renderItem={(item) => (
              <List.Item key={item.id}>
                <NotificationItemWrapper
                  onClick={() => navigate(item.link, item.notifid)}
                >
  ...
  const renderCircle = (title, circleid) => ({
    ...
    label: (
      <div
        ...
        onClick={() => history.push(`/my-circles/${circleid}/all-posts`)}
      >
        {title}
      </div>
    ),
  ...
  const handleSearchChange = (values) => {
    ...
    const { Option } = Select;
    if (values) {
      axios.get(`${Url}/users/search/${values}`).then((res) => {
        const options = [
          {
            label: renderTitle("Users"),
            options: res.data.map((user) => renderItem(user.name, user.userid)),
          },
        ];
      ...
      axios.get(`${Url}/circles/search/${values}`).then((res) => {
        const options = [
          {
            label: renderTitle("Circles"),
            options: res.data.map((circle) =>
      ...
      setTotalOptions(userOptions.concat(circleOptions));
    } else {
      setTotalOptions(null);
    }
  };
